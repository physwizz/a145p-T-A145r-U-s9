	...
	uint32_t *pu4QueryInfoLen)
{
	...
	ucSize = sizeof(struct CMD_TX_POWER_COMPOSITION_T) +
		sizeof(struct PARAM_TXPOWER_COMPOSITION_T) * ucNum;
	prCmdTxPCompos =
		(struct CMD_TX_POWER_COMPOSITION_T *) kalMemAlloc(ucSize,
		VIR_MEM_TYPE);

	kalMemSet(prCmdTxPCompos, 0, ucSize);
	...
	kalMemFree(prCmdTxPCompos, VIR_MEM_TYPE, ucSize);
